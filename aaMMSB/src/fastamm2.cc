...
FastAMM2::FastAMM2(Env &env, Network &network)
  :_env(env), _network(network),
   ...
   _eta(_k,_t),
   _hitcurve_id(0), _inf_epsilon(0.5),
   _link_thresh(0.9), _total_pairs_sampled(0),
   ...
   _neighbors(_env.reportfreq)
{
  ...
  Env::plog("non-informative sets per node", _m);
  ...
  Env::plog("stratified random node, alpha:", env.alpha);
  Env::plog("stratified random node, eta0:", env.eta0);
  Env::plog("stratified random node, eta1:", env.eta1);
  Env::plog("stratified random node, tau0:", _tau0);
  Env::plog("stratified random node, kappa:", _kappa);
  Env::plog("stratified random node, nodetau0:", _nodetau0);
  Env::plog("stratified random node, nodekappa:", _nodekappa);
  Env::plog("stratified random node, epsilon:", _env.epsilon);
  ...
  if (!_env.load_heldout) {
    Env::plog("stratified random node, load heldout from file:", false);
    init_heldout();
  } else {
  ...
  if (_env.model_load) {
    assert(load_model() >= 0);
    Env::plog("stratified random node, load gamma from file:", true);
    Env::plog("stratified random node, initial lambda", _lambda);
  } else {
    init_gamma();
    assert (init_lambda() >= 0);
    Env::plog("stratified random node, load gamma from file:", false);
    Env::plog("stratified random node, initial lambda", _lambda);
  }
  ...
  info("+ done computing heldout likelihood\n");
...
FastAMM2::init_heldout()
{
  ...
  if (!_env.single_heldout_set) {
    ...
    info("set validation samples done\n");
  }
...
string
FastAMM2::edgelist_s(EdgeList &elist)
{
  ostringstream sa;
...
void
FastAMM2::set_heldout_sample(int s)
{
  int c0 = 0;
  int c1 = 0;
  int p = s / 2;
...
void
FastAMM2::set_validation_sample(int s)
{
...
#ifdef TRAINING_SAMPLE
void
FastAMM2::set_training_sample(int s)
{
  int c0 = 0;
  int c1 = 0;
  int p = s / 2;
  ...
  while (1) {
	...
	for (uint32_t k = 0; k < _k; ++k)
	  gd[i][k] = (1 - _noderhot[i]) * gd[i][k] +		\
	    _noderhot[i] * (_alpha[k] + scale * gdt[i][k]);
      } else {
	//set_dir_exp(i, _gamma, _Elogpi);// for perf cmp reasons
	//_gammat.zero(i);
	for (uint32_t k = 0; k < _k; ++k)
	  gd[i][k] = (1 - _noderhot[i]) * gd[i][k] +		\
...
void
FastAMM2::compute_and_log_groups()
{
  ...
  if (_env.nmi) {
    if (!_env.benchmark) {
      ...
      if (system(cmd) < 0)
	lerr("cmd %s returned with error %s", cmd, strerror(errno));
    } else {
      char cmd[1024];
      sprintf(cmd, "/u/pgopalan/mutual %s %s >> %s",
 		      ...
 		      FreqMap &counts)
{
  ...
  vector<uint32_t> *zeros = NULL; // _network.sparse_zeros(_start_node);
			     ...
			     FreqMap &counts)
{
  ...
  // with respect to start node;
  // 'i' is the number of informative nodes of start node;
  // there are 'm' such sequences
  // since n >> i, n - i / m is approx. equal to n / m
  uint32_t setsize = (double)_n / (double)_m;

  double v = (double)(gsl_rng_uniform_int(_r, _n)) / setsize;
  uint32_t q = ((int)v) * setsize;
  tst("\nq = %d, set size = %d\n", q, setsize);

  while (sample.size() < setsize) {
...
double
FastAMM2::approx_log_likelihood()
{
  ...
  for (uint32_t p = 0; p < _n; ++p) {
	...
	continue;
      ...
      Array Elogf(_k);
      PhiCompute::compute_Elogf(p,q,y,_k,_t,_Elogbeta,Elogf);
...
void
FastAMM2::compute_precision(uint32_t &c10, uint32_t &c100,
			   uint32_t &c1000)
{
  if (_env.accuracy)
    return;
...
double
FastAMM2::validation_likelihood()
{
  if (_env.accuracy || _env.single_heldout_set)
    return .0;
...
void
FastAMM2::compute_adamic_adar_score()
{
...
void
FastAMM2::moving_heldout_likelihood(EdgeList &sample) const
{
  if (_env.accuracy)
    return;
...
void
FastAMM2::group_stats(const uArray &nodes, uint32_t M)
{
...
void
FastAMM2::write_node_gml(FILE *f, uint32_t a, uint32_t max_k)
{
  if (max_k < 65536)
...
inline uint32_t
FastAMM2::most_likely_group(uint32_t p)
{
  Array Epi(_k);
  get_Epi(p, Epi);
...
double
FastAMM2::bridgeness(uint32_t a)
{
  Array Epi(_k);
  get_Epi(a, Epi);
